...
def initialize_db_connection(
    ...
    if db_version == 2:
        client = influx_v2_service.create_client(
            configuration.influx_db_server_config, password
        )
    # elif db_version == 1:
    #     client = influx_v1_service.create_client(configuration.influx_db_settings, password)
    else:
        _logger.error(
            f"Unsupported InfluxDB version '{db_version}'. Only version 1 is supported."
        )
