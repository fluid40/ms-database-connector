...
from ms_database_connector.models.exceptions.configuration_errors import ConfigurationError
...
class TestSetupMappingService:
    ...
    def test_mapping_provider_exception_propagates(self):
        ...
        state = _init_startup_state()
        runtime_deps = MagicMock()
        runtime_deps.mapping_handler_provider.side_effect = ConfigurationError("Bad mapping")
...
class TestSetupAasConnection:
    ...
    def test_connection_error_propagates(self):
        ...
        runtime_deps = MagicMock()
        mock_handler = MagicMock()
        mock_handler.connect_to_server.side_effect = ConnectionError("Server unreachable")
        runtime_deps.server_handler_factory.return_value = mock_handler
        runtime_deps.config_loader_factory.return_value = MagicMock()
...
class TestLifespanIntegration:
    ...
    async def _run_lifespan_context(self, runtime_deps):
        ...
        with patch("ms_database_connector.__main__.build_app_runtime_deps", return_value=runtime_deps):
    ...
    async def test_happy_path_startup_all_steps_succeed(self):
        ...
        with patch("ms_database_connector.__main__.build_app_runtime_deps", return_value=runtime_deps):
            with patch("ms_database_connector.__main__._preload_aas_metadata") as mock_preload:
